...
| `--oref` | 万物参考（Omni-Reference） | 图片URL | V7+ 新特性，任意物体/角色/材质参考，可替代部分 --cref 场景 |
| `--exp` | 实验性美学 | 0-100 | V7+ 探索性风格渲染 |
| 文字渲染 | 用引号标注文字 | `"文字内容"` | V8 官方：引号标注时文字渲染效果显著改善 |

> 版本说明（2026-07 校准）：V8.1 已成默认模型，V8.2 已发布。V8 官方建议"更长、更具体的提示词"，与 V6 时代"短提示+标签堆砌"写法相反；个性化（Personalization）默认开启，建议 stylize 可拉高至 1000 实验。
...
### 3.3 GPT-Image / FLUX / 国产模型参数速查（2025-2026 新增）

#### GPT-Image（GPT-4o）

| 要点 | 说明 |
|------|------|
| 写法 | 原生多模态，指令遵循与文字渲染强；具体明确+说明用途上下文 |
| 复杂需求 | 分步指令（先背景→再文字→后物体），多轮对话迭代修正优于一次写完美长句 |
| 负面提示 | 无此机制，一律用正向描述 |
...
| 要点 | 说明 |
|------|------|
| 结构 | 主体→动作→风格→上下文，**词序即优先级** |
| 长度分档 | 10-30词探索 / 30-80词常规 / 80+词复杂场景 |
| 文字渲染 | 引号+位置+字体风格+HEX色值（如 'ACME' in #FF5733） |
| JSON 结构化 | 支持 scene/subjects/camera/color_palette 字段，适合生产流水线 |
| 负面提示 | **官方不支持，勿用** |
| 语言 | 建议用母语书写；可用相机语言（shot on Sony A7IV） |

#### 即梦 4.0 / 可灵（国产）

| 要点 | 说明 |
|------|------|
| 语言 | 中文大白话原生支持，无需翻译成英文 |
| 文生图公式 | 主体+行为+环境 / 风格+色彩+光影+构图（即梦官方） |
| 编辑公式 | 变化动作+变化对象+变化特征；**必写"保持 XX 不变"** |
| 多图玩法 | 组合/迁移/替换/组图四种（即梦最多9张组图） |
| 可灵公式 | 5W1H（Who/What/When/Where/Why/How），但"不必写全、避免盲目堆叠" |
| 避坑 | 指代模糊、指令过泛、缺保持描述、风格矛盾（官方问题清单点名） |

#### 负面提示词分平台策略

| 平台 | 策略 |
|------|------|
| Stable Diffusion / ComfyUI | ✅ 保留负面提示词，逐条验证、删冗余项 |
| Midjourney | ⚠️ 用 --no 参数（如 --no people text），正文不写否定句；效果"有限且不可预测" |
| FLUX / GPT / Nano Banana | ❌ 不支持/无机制，一律改为正向描述（"空旷无人的街道"） |
...
│
├── 图生图编辑 / 局部修改 / 多图合成
│   └── → Nano Banana 2/Pro 或 即梦4.0（对话式编辑："单点+具体+保持描述"）
│
├── 生产流水线 / 批量结构化生成
│   └── → FLUX.2（JSON 结构化提示，词序即优先级，禁负面词）
│
├── 国产平台 / 纯中文环境 / 组图故事
│   └── → 即梦 4.0 / 可灵（中文原生，组图最多9张）
│
└── 创意构思 / 风格探索 / 灵感发散
...
## 6. Nano Banana 官方场景模板与图生图玩法（2026-07 新增）

> 来源：@AI绘画提示词 知识库 — Google 官方提示词手册 + Nano Banana 2 全网最全风格指南（2025-11 ~ 2026-03）

### 6.1 官方六大场景模板

| 场景 | 提示词结构 | 示例（中文） |
|------|-----------|-------------|
| 🎬 写实摄影 | 镜头类型+主体+动作/表情+环境+光线+相机参数+比例 | 一张超写实特写肖像照，一位日本老年陶艺家正在端详茶碗，光线透过窗户洒落，85毫米镜头，16:9 |
| 🎨 插画/贴纸 | 风格+主体+关键特征+配色+线条风格+背景要求 | 一张可爱风格贴纸，戴竹帽的红熊猫，粗线条轮廓，赛璐珞阴影，白色背景 |
| 📝 文字渲染 | 图像类型+品牌概念+文本内容+字体风格+设计风格+配色 | 为"TechFlow"创建极简主义标志，简洁无衬线字体，融合集成电路图标，黑白两色 |
| 📷 产品摄影 | 产品描述+背景+布光+机位角度+关键细节+比例 | 工作室级产品摄影，哑光黑色陶瓷马克杯置于混凝土表面，三点式布光，45度角，蒸汽聚焦 |
| 🌸 极简留白 | 主体位置+背景留白+光线+比例 | 极简构图，单片红枫叶位于右下角，大面积米白留白，左上角柔和漫射光，方形画幅 |
| 💬 连续艺术（漫画/故事板） | 画风+前景人物+背景场景+对话框文本+氛围 | 黑色电影风漫画分格，侦探站在路灯下，霓虹酒吧倒影，配文"这座城市从不入眠"，戏剧性布光 |

### 6.2 图生图七大玩法（图片+文字→新图）

| 玩法 | 指令写法要点 | 示例 |
|------|-------------|------|
| 1. 增删元素 | 直接说改什么，模型自动保持风格/光照/透视一致 | "去除图中手写痕迹" |
| 2. 局部重绘（语义遮盖） | 用对话描述"蒙版"位置，保持其余部分不变 | "把沙发换成深蓝色丝绒款，其余保持不变" |
| 3. 风格迁移 | 提供图片+指定新艺术风格 | "把这张照片重绘成水彩风格" |
| 4. 多图合成 | 显式分配每张图角色 | "提取图1的蓝色碎花连衣裙，让图2的女性穿上，生成全身实景照，调整光影与户外环境一致" |
| 5. 高保真物体迁移 | 强调"面部/特征完全不变" | "把图2的标识添加到她的黑色T恤上，确保面部特征完全不变，标识像自然印制在面料上" |
| 6. 草图成品化 | 保留草图特征+补充成品细节 | "把这张汽车铅笔草图转化为展厅概念车照片，保留流畅线条与低趴姿态，添加金属蓝车漆和霓虹轮毂灯" |
| 7. 角色360°视图 | 迭代提示不同角度，把已生成图追加为参考保持一致性 | "基于上一张图生成同一角色的背面视图" |

### 6.3 Nano Banana 2 六大风格公式库

| 类别 | 公式要素 | 代表配方 |
|------|---------|---------|
| 📸 超写实（PBR渲染管线） | 布光方案+镜头焦段+胶片/渲染质感 | 三点布光+无缝背景+85mm人像镜头压缩+眼神光；35mm胶片颗粒+柔和高光滚降 |
| 💰 商业广告 | 背景+戏剧性布光+材质强调+情绪 | 深黑背景+戏剧性轮廓光+反光台面+高级材质（奢华暗调）；纯白无缝背景+可控阴影衰减（电商标准） |
| 🎨 矢量插画 | 着色方式+线条+几何化+纸感 | 平滑渐变+高饱和+锐利边缘（数字精密）；水彩晕染+纸张纹理可见（手绘感） |
| 🕰️ 复古怀旧 | 年代色板+介质质感+文化符号 | 1980s：热粉+电光蓝+网格透视+日落渐变（Synthwave）；1930s：重胶片颗粒+黑白对比（黑色电影） |
| 🛠️ 图像重混 | 指令式编辑动词+保持项 | "用黄金时刻暖调重打光，加长阴影和琥珀色偏"；"应用青橙好莱坞调色，暖肤色冷阴影" |
| 📊 数据可视化 | 图表类型+版式+视觉风格 | 垂直时间轴信息图+极简版式+现代矢量图标；等距3D流程图+发光连接线+暗色模式 |

### 6.4 文字渲染四要素（全平台通用）

需要画面出现文字时必写全：**引号确切文字 + 字体风格 + 颜色 + 位置**。
- Nano Banana：中文需强调 2K/4K；支持多语言翻译本地化（"把图上英文全翻译成中文，其他不变"）
- MJ V8：引号标注时文字渲染显著改善
- FLUX.2：可加 HEX 色值精确定位
- 避坑：避免小字号长文本；文字越少越准确
